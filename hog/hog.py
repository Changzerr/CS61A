...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0,score1) < goal:
        ...
        curScore = current_scores[player]
        opponent = other(player)
        oppScore = current_scores[opponent]
        times = 0
        if player == 0:
            times = strategy0(curScore,oppScore)
        else:
            times = strategy1(curScore,oppScore)

        scores = take_turn(times,oppScore,dice)
        if player == 0:
            score0 += scores
        else:
            score1 += scores
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(score0, score1):
        curScore = (score0,score1)[who]
        gain = curScore - previous_score
        ...
        return announce_highest(who,new_high,curScore)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    while current_dice_number <=10:
        current_socre = make_averaged(roll_dice,num_samples)(current_dice_number,dice)
        if current_socre > highest_average_score:
            highest_average_score = current_socre
            highest_dice_number = current_dice_number 
        elif highest_average_score == current_socre and current_dice_number < highest_dice_number:
            highest_dice_number = current_dice_number
        current_dice_number += 1
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 10
    current_socre = free_bacon(opponent_score)
    if current_socre >= margin:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    current_socre = score + bacon_socre
    if is_swap(current_socre,opponent_score):
        if opponent_score > current_socre:
            return 0
    elif bacon_socre >= margin:
        return 0
    return num_rolls
    # END PROBLEM 11
